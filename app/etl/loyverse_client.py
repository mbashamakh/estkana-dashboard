...
def list_receipts_page(
    ...
    strings (e.g. "2026-01-01T00:00:00.000Z"). `store_id` is an optional
    server-side filter to one Loyverse store (see loyverse_store_map) —
    added for the Est-010/Forosia sale-consumption pull, so a single-branch
    query doesn't have to page through every other branch's receipts too.
    Returns the raw response dict — {"receipts": [...], "cursor": "..." or None}.
    """
    return _get(
        settings, "/receipts",
        {
...
def list_all_receipts(
    ...
    API — not expected to ever be hit in practice. `store_id` optionally
    narrows to one store server-side (see list_receipts_page).
    """
    receipts: list[dict] = []
    cursor: str | None = None
    for _ in range(max_pages):
        page = list_receipts_page(
            ...
            cursor=cursor, limit=250, store_id=store_id,
        )
